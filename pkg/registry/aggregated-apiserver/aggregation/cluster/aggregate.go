...
func (r *ProxyREST) connectAllClusters(
	...
	if len(proxyRequestInfo.Name) != 0 {
		return r.requestWithResourceNameHandlerFunc(ctx, proxyPath, responder, clusterList), nil
	}
	return r.requestWithoutResourceNameHandlerFunc(ctx, proxyPath, responder, clusterList), nil
}
	...
	return http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		...
		for i := range clusterList.Items {
			...
			go func(cluster *apis.FederatedCluster) {
				...
				statusCode, err := doClusterRequest(req.Method, requestURLStr(location.String(), proxyRequestInfo), transport, requester)
				if err != nil {
		...
		case 1:
			...
			setRequestHeader(req, requester)
	...
	return http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		...
		for i := range clusterList.Items {
			...
			go func(cluster *apis.FederatedCluster) {
				...
				setRequestHeader(simpleRequest, requester)
...
func doClusterRequest(
	...
	setRequestHeader(simpleRequest, userInfo)
...
func setRequestHeader(req *http.Request, userInfo user.Info) {
